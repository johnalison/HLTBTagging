...
class CombinedSVComputer {
	...
	template <typename T> 
	  const reco::Track& getRecoTrack(const T* track) const;
...
template <typename T> 
const reco::Track&  CombinedSVComputer::getRecoTrack(const T* track) const{
  return *(*track);
}

template <>
inline const reco::Track& CombinedSVComputer::getRecoTrack<edm::Ptr<reco::Candidate> >(const edm::Ptr<reco::Candidate>* track) const{
  return *reco::btag::toTrack(*track);
}
...
//              vars.insert(btau::trackGhostTrackSig, data.distanceToGhostTrack.significance(), true);
                ...
                vars.insert(btau::trackPhi, trackMom.Phi(), true);

		const reco::Track& recoTrack = getRecoTrack<TrackRef>(&track);
		vars.insert(btau::trackNTotalHits, recoTrack.hitPattern().numberOfValidHits(), true);
		vars.insert(btau::trackNPixelHits, recoTrack.hitPattern().numberOfValidPixelHits(), true);
		vars.insert(btau::trackChi2,       recoTrack.normalizedChi2(), true);
		vars.insert(btau::trackCharge,     recoTrack.charge(), true);

		const reco::HitPattern &p = recoTrack.hitPattern();
		bool hitInInnerLayer = p.hasValidHitInPixelLayer(PixelSubdetector::SubDetector::PixelBarrel, 1);
		vars.insert(btau::trackJetDistSig, hitInInnerLayer, true);		
		
		std::cout << "algo " << recoTrack.algo() << " originalAlgo " << recoTrack.originalAlgo() << std::endl;
		vars.insert(btau::trackP0Par, recoTrack.algo(), true);		
		vars.insert(btau::trackP0ParRatio, recoTrack.originalAlgo(), true);		
