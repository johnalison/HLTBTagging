...
def flavourComposition(loglev, doData, doCSV, doDeepCSV, plotinclusive, plotTagAndProbe, calcEfficiency, TagWPCSV = 0.9535, TagWPDeepCSV = 0.8958):
    import ROOT
    ...
    globalPrefix = "DeepCSVMPresel_phase1_RunC"
    basepath = "v5nTuples/FlavourSplitting/"
    ...
    MCsamples.append( modules.classes.Sample("ttbar_unmatchedJets", MCInput, "{0} && (  abs(offJets_mcFlavour[?]) != 21 && abs(offJets_mcFlavour[?]) > 5 )".format(eventSelection), 831.76, datalumi, ROOT.kWhite, 9767140, weight = "get_puWeight(pu)", legendText = "unmatched jets (t#bar{t})") )
    MCsamples.append( modules.classes.Sample("ttbar_udsgJets", MCInput, "{0} && ( abs(offJets_mcFlavour) == 21 || abs(offJets_mcFlavour) < 4)".format(eventSelection), 831.76, datalumi, ROOT.kBlue, 9767140, weight = "get_puWeight(pu)", legendText = "udsg jets (t#bar{t})") )
    MCsamples.append( modules.classes.Sample("ttbar_cJets", MCInput, "{0} && abs(offJets_mcFlavour) == 4".format(eventSelection), 831.76, datalumi, ROOT.kGreen+2, 9767140, weight = "get_puWeight(pu)", legendText = "c jets (t#bar{t})") )
    MCsamples.append( modules.classes.Sample("ttbar_bJets", MCInput, "{0} && abs(offJets_mcFlavour) == 5".format(eventSelection), 831.76, datalumi, ROOT.kRed, 9767140, weight = "get_puWeight(pu)", legendText = "b jets (t#bar{t})") )

    MCsamplesIter.append( modules.classes.Sample("ttbar_unmatchedJets", MCInput, "{0} && ( abs(offJets_mcFlavour[?]) != 21 && abs(offJets_mcFlavour[?]) > 5 )".format(eventSelection), 831.76, datalumi, ROOT.kWhite, 9767140, weight = "get_puWeight(pu)", legendText = "unmatched jets (t#bar{t})") )
    MCsamplesIter.append( modules.classes.Sample("ttbar_udsgJets", MCInput, "{0} && ( abs(offJets_mcFlavour[?]) == 21 || abs(offJets_mcFlavour[?]) < 4)".format(eventSelection), 831.76, datalumi, ROOT.kBlue, 9767140, weight = "get_puWeight(pu)", legendText = "udsg jets (t#bar{t})") )
    MCsamplesIter.append( modules.classes.Sample("ttbar_cJets", MCInput, "{0} && abs(offJets_mcFlavour[?]) == 4".format(eventSelection), 831.76, datalumi, ROOT.kGreen+2, 9767140, weight = "get_puWeight(pu)", legendText = "c jets (t#bar{t})") )
    MCsamplesIter.append( modules.classes.Sample("ttbar_bJets", MCInput, "{0} && abs(offJets_mcFlavour[?]) == 5".format(eventSelection), 831.76, datalumi, ROOT.kRed, 9767140, weight = "get_puWeight(pu)", legendText = "b jets (t#bar{t})") )
    ...
    if plotTagAndProbe:
        ...
        if doCSV:
            WPlabel = getLabel("Tag CSV WP: {0}".format(TagWPCSV),  styleconfig.getfloat("CMSLabel","xStart"), scale = 0.8)
            ...
            if calcEfficiency:
                ...
                modules.TagNProbe.getBEfficiency(PFCSVnthJet, MCsamplesIter, "{0} && {1}".format(probeSel, "offJets_matchPF[?] >= 0"), tagSel,
                                                 ...
                                                 outname = basepath+globalPrefix+"_TnP_leading_pf_csv", label = [WPlabel])
                ...
                #                               "caloJets_csv[offJets_matchCalo[?]]", [0.42, 0.840, 0.97], 3, data = dataSample)
            else:
        ...
        if doDeepCSV:
            ...
            WPlabel = getLabel("Tag DeepCSV WP: {0}".format(TagWPDeepCSV),  styleconfig.getfloat("CMSLabel","xStart"), scale = 0.8)
            ...
            if calcEfficiency:
                ...
                modules.TagNProbe.getBEfficiency(PFDeepCSVnthJet, MCsamplesIter, "{0} && {1}".format(probeSel, "offJets_matchPF[?] >= 0"), tagSel,
                                                 "pfJets_deepcsv[offJets_matchPF[?]]", [0.205, 0.67, 0.955], 3, data = dataSample)
                logging.info("Calculating Calo Efficiency")
                modules.TagNProbe.getBEfficiency(CaloDeepCSVnthJet, MCsamplesIter, "{0} && {1}".format(probeSel, "offJets_matchCalo[?] >= 0"), tagSel,
                                                 "caloJets_deepcsv[offJets_matchCalo[?]]", [0.205, 0675., 0.95], 3, data = dataSample)
            else:
