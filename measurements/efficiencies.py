...
def efficiencies(loglev, doMC, doData, doCSV, doDeepCSV, CSVWPs, DeepCSVWPs, nJets = 4):
    import ROOT
    ...
    if doCSV:
        ...
        for WP in CSVWPs:
            ...
            for i in range(nJets):
                ...
                CSVPlotBaseObjs[i] = modules.classes.PlotBase("offJets_csv",
                                                              "{0} && {1} && offJets_csv >= 0 && offJets_rankCSV == {2}".format(CSVSelection, offlineSelection, i),
                                                              "1",
                                                              [20,0,1],
                ...
                for sampleStuff in samples:
                    ...
                    if not (doMC and doData): 
                        modules.effPlots.makeEffPlot(CSVPlotBaseObjs[i], sample, "pfJets_csv >= {0}".format(WP),
                                                     basepaths+"Eff_{2}_Jet{0}_pfWP_{1}_CSV".format(i, WP, name), label = [label,WPLabel])

                if doMC and doData:
                    modules.effPlots.makeEffSCompPlot(CSVPlotBaseObjs[i], onlysamples, "pfJets_csv >= {0}".format(WP),
    ...
    if doDeepCSV:
        ...
        for WP in DeepCSVWPs:
            ...
            for i in range(nJets):
                ...
                DeepCSVPlotBaseObjs[i] = modules.classes.PlotBase("offJets_deepcsv",
                                                              "{0} && {1} && offJets_deepcsv >= 0 && offJets_rankDeepCSV == {2}".format(DeepCSVSelection, offlineSelection, i),
                                                              "1",
                                                              [20,0,1],
                ...
                for sampleStuff in samples:
                    ...
                    onlysamples.append(sample)
                    if not (doMC and doData): 
                        modules.effPlots.makeEffPlot(DeepCSVPlotBaseObjs[i], sample, "pfJets_deepcsv >= {0}".format(WP),
                                                     basepaths+"Eff_{2}_Jet{0}_pfWP_{1}_DeepCSV".format(i, WP, name), label = [label,WPLabel])

                if doMC and doData:
                    modules.effPlots.makeEffSCompPlot(DeepCSVPlotBaseObjs[i], onlysamples, "pfJets_deepcsv >= {0}".format(WP),
